...
def main() -> None:
    ...
    for idx in sample_indices:
        ...
        print(f"\n[Sample Row #{idx + 1:,}]")
        print(
            f"  RAW SOURCE:  Pickup={raw_match['tpep_pickup_datetime']} | Dropoff={raw_match['tpep_dropoff_datetime']} | Dist={raw_match['trip_distance']} mi | PULocationID={raw_match['PULocationID']} | DOLocationID={raw_match['DOLocationID']} | Fare=${raw_match['fare_amount']:.2f}"
        )
        print(
            f"  TRANSFORMED: Duration={clean_row['trip_duration_seconds']:.0f}s ({clean_row['trip_duration_seconds']/60:.1f}m) | 15m_Bin={clean_row['time_bin_15m']} | Dist={clean_row['trip_distance_km']:.2f} km | PU_Zone={clean_row['pickup_zone_id']} | DO_Zone={clean_row['dropoff_zone_id']} | Speed={clean_row['average_speed_mph']:.1f} mph | Weekend={clean_row['is_weekend']} | TripID={clean_row['trip_id']}"
        )
