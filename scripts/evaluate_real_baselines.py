"""Evaluate Seasonal-Naive Baselines on full-scale 2023-01 real validation split (M3-3)."""
...
def main():
    print("=== M3-3: Real Seasonal-Naive Baseline Evaluation on 2023-01 Data ===")
    ...
    tmp_dir = tempfile.mkdtemp(prefix="baseline_eval_")
    try:
        ...
        # 3. Demand Baseline Evaluation
        print("\n--- Step 3: Evaluating Demand Seasonal-Naive Baseline ---")
        ...
        demand_eval = evaluate_demand_baseline(
            ...
            run_name="seasonal_naive_demand_validation_benchmark",
            log_to_mlflow=True,
        )
        d_metrics = demand_eval["metrics"]
        print("\n=======================================================")
        print("DEMAND SEASONAL-NAIVE BASELINE RESULTS (Validation Split):")
        print(
            "  Validation Window: Jan 25, 2023 00:00 UTC - Jan 31, 2023 23:00 UTC (7 days)"
        )
        print(
            f"  Validation Samples: {int(d_metrics['val_sample_count']):,} zone-hours"
        )
        print(f"  MAE:                {d_metrics['val_mae']:.4f} pickups/hour")
        ...
        print(f"  MLflow Run ID:      {demand_eval['run_id']}")
        print("=======================================================")

        # 4. Corridor Duration Baseline Evaluation
        print("\n--- Step 4: Evaluating Corridor Duration Baseline ---")
        ...
        corridor_eval = evaluate_corridor_duration_baseline(
            ...
            run_name="moving_average_duration_validation_benchmark",
            log_to_mlflow=True,
        )
        c_metrics = corridor_eval["metrics"]
        print("\n=======================================================")
        print("CORRIDOR DURATION BASELINE RESULTS (Validation Split):")
        print(
            "  Validation Window:  Jan 25, 2023 00:00 UTC - Jan 31, 2023 23:00 UTC (7 days)"
        )
        print(
            f"  Validation Samples: {int(c_metrics['val_sample_count']):,} corridor-hours"
        )
        print(
            f"  MAE:                {c_metrics['val_mae']:.2f} seconds ({c_metrics['val_mae']/60.0:.2f} minutes)"
        )
        print(
        ...
        print(f"  MLflow Run ID:      {corridor_eval['run_id']}")
        print("=======================================================")
