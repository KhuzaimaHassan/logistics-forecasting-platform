...
**What didn't, and why:**
- *The `-s ours` Merge Investigations & Core Caveat:*
  - *The Real Lesson / Invariant (Caveat as Headline):* `git merge -s ours origin/main` is **only safe when you have independently verified that `main`'s content is a strict subset of (or byte-identical to) `dev`'s; it does not verify this itself, so check the diff (`git diff origin/main..dev`) every time before using it.** The `-s ours` strategy is a blunt graph-repair mechanism that blindly and silently discards all changes introduced on the merged branch (`origin/main`) without inspecting them or emitting conflict warnings. If an emergency hotfix or direct commit ever lands on `main` that is not already in `dev`, running `-s ours` will permanently obliterate that change from `dev` without a trace. It is never a general merge strategy, but a surgical fix for stranded squash-merge ancestors under strictly verified subset conditions.
  ...
  - *Merge Investigation 2 (PR #103 → PR #104, PR #106 → PR #107, PR #109 → PR #110):* To solve the stranded merge-base problem without manual conflict resolution, the team investigated `git merge -s ours origin/main`. Because PR #99 had originated entirely from `dev` and had no independent commits on `main`, the team independently verified that `main` was a strict subset of `dev`. In that specific verified state, `-s ours` safely recorded the squash commit from `main` as a parent of `dev` without altering a single byte of `dev`'s working tree, advancing the merge-base between `dev` and `main` to the tip of `main`. Subsequent PRs from `dev` to `main` became clean fast-forwards with zero conflicts. This was proven in PR #104 (`534bdc0`), repeated in PR #107 (`8b804f5`) and PR #110 (`1c50b16`), and formalized as a standing post-squash sync rule in `docs/GitHub-Setup.md` — but strictly conditioned on the mandatory diff pre-check.
...
**What would change if starting this phase over:**
- Establish the verified post-squash sync protocol (`git diff origin/main..dev` verification followed by `git merge -s ours origin/main`) from milestone 1 instead of discovering the squash-merge ancestor divergence the hard way in Phase 4.
